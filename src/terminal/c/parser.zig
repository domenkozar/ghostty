...
const terminal = @import("../Parser.zig");
...
pub const CSI = extern struct {
    ...
    comptime {
        ...
        if (terminal.MAX_PARAMS > 32) @compileError("MAX_PARAMS exceeds params_sep u32 capacity");
    }
};
...
fn convertAction(wrapper: *Wrapper, action: terminal.Action) Action {
    return switch (action) {
        ...
        .csi_dispatch => |c| csi: {
            wrapper.last_csi = .{
                ...
                .final = c.final,
                .params_sep = @truncate(c.params_sep.mask),
            };
